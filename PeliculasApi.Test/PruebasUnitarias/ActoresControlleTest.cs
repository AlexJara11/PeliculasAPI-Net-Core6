...
{
    [TestClass]
    public class ActoresControlleTest: BasePruebas
    {
        [TestMethod]
        ...
        [TestMethod]
        public async Task PatchRetorna404SiNoExiste()
        {
            var nombreBD = Guid.NewGuid().ToString();
            var contexto = ConstruirContext(nombreBD);
            var mapper = ConfigurarAutoMapper();
        ...
        [TestMethod]
        public async Task PatchActualizaUnCampo()
        {
            var nombreBD = Guid.NewGuid().ToString();
            var contexto = ConstruirContext(nombreBD);
            var mapper = ConfigurarAutoMapper();
            ...
            var patchDoc = new JsonPatchDocument<ActorPatchDTO>();
            patchDoc.Operations.Add(new Microsoft.AspNetCore.JsonPatch.Operations.Operation<ActorPatchDTO>("replace", "/nombre", null, "Claudia"));
            var respuesta = await controller.Patch(1, patchDoc);
            var resultado = respuesta as StatusCodeResult;
